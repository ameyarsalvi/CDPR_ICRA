classdef PRPRcommon
  ...
  methods 
      ...
      function Y = fwd_dynamics(obj,X_dyn,l,cable_tensions,dt,tf)
          jd = 1;
          q = 0.001;
          r = 0.000002;
          for t_dyn = dt:dt:tf
        
            P_x = obj.structureMatrix(X_dyn(1:3,jd),l);
            x_dot = X_dyn(4:6,jd);
            x_ddot = obj.M\(P_x*cable_tensions) - obj.M\obj.D*x_dot;
            
            X_dyn(1:3,jd+1) = X_dyn(1:3,jd) + x_dot*dt; 
            X_dyn(4:6,jd+1) = x_dot + x_ddot*dt;

            Y(:,jd+1) = X_dyn(:,jd+1); %sqrt(r)*randn(6,1);
            jd = jd+1;
          end

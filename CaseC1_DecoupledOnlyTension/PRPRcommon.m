classdef PRPRcommon
  ...
  methods 
      ...
      function [Y,ls0,T] = fwd_dynamics(obj,X_dyn,ls0,ls_ref,T0,T_des,dt,tf)
        %m = 0.05;
        %d = 0.4;
        %gp = 0.9;
        jd = 1;
        ...
        for t_dyn = dt:dt:tf
            ...
            x_dot = X_dyn(4:6,jd);
            %T = T0 + deltaT.*(ones(4,1)-(exp(-t_dyn/0.6).*ones(4,1)));
            T = T_des;
            x_ddot = obj.M\(P_x*T) - obj.M\obj.D*x_dot;% + sqrt(q)*randn(3,1)*sqrt(dt); % process noise

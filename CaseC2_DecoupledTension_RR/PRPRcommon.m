classdef PRPRcommon
  ...
  methods 
      ...
      function [X_dyn,l1] = fwd_dynamics(obj,X_dyn,l0,l_des,T_des,dt,tf)
          jd = 1;
          l1(:,1) = l0;
          l_dot = (l_des - l0)/tf;
          l_dot = min(max(l_dot,-0.05),0.05);
          for t_dyn = dt:dt:tf
            
            l1(:,jd+1) = l1(:,jd) + l_dot*dt;

            P_x = obj.structureMatrix(X_dyn(1:3,jd),l1(:,jd));
            x_dot = X_dyn(4:6,jd);
            x_ddot = obj.M\(P_x*T_des) - obj.M\obj.D*x_dot;

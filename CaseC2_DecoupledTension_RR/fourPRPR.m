...
%       function kappa = sensitivity_kappa(obj,X_vec,l)
%           for i = 1:1:size(X_vec,2)
%             Jw = obj.structureMatrix(X_vec(1:3,i),l(:,i));
%             z = null(Jw);
%             if min(z) > 0
%                 kappa(i) = min(z)/max(z);
%             elseif max(z) < 0
...
%       end
      ...
      function [theta,l_new,tau_pos] = joint_state(obj,X,X_dot,theta0,l0,dt,tau_pos_init)
        ...
        %l_new = l0 + l_dot*dt;
        % Applying FOL on slider 
        Delta_l = (l_des - l0);
        k = abs(6*Delta_l); % 0.3;
        l_new = l0 + Delta_l.*(ones(4,1)-exp(-dt./k));
